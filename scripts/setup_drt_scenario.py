...
def write_cut_config(
    ...
    The synpp `matsim.simulation.cutter.cut` stage has two upstream bugs
    that prevent it from producing usable output in this fork:
      1. Passes relative paths AND sets cwd=output_path, so Java resolves
         `--config-path output_path/prefix_config.xml` as
         `output_path/output_path/prefix_config.xml` — not found.
      2. `data/cutter/geometry.py` writes the .shp in WGS84 (inherits from
         the .geojson CRS) while the MATSim network is in Lambert-93; Java
         finds zero network intersections.
    Rather than patching both upstream files, we skip the synpp cutter stage
    entirely and run the cutter manually via scripts/run_cutter.sh, which
    uses absolute paths + the L93 companion shapefile that
    build_cutter_polygon.py emits alongside the GeoJSON.
    """
...
## Uses IDF default mode-choice parameters (Hörl & Balac 2021). No calibration.
##
## synpp only produces the prepared full-region MATSim XMLs at this sample rate.
## The 40 km cut is applied afterwards by scripts/run_cutter.sh — see docstring
## in scripts/setup_drt_scenario.py for why we skip synpp's cutter stage.
...
def write_phase_a_config(path: Path) -> None:
    """Full-region 10% scenario artifacts for Phase A travel-time extraction.

    NOTE: run_matsim is FALSE here. eqasim's matsim.simulation.run stage
    hardcodes lastIteration=1 (see matsim/simulation/run.py:21) which isn't
    enough for stable congested travel times. Instead, synpp only produces
    the prepared MATSim XMLs; the actual multi-iteration travel-time run is
    driven by scripts/run_travel_time_extraction.sh.
    """
    content = """## Lyon Phase A — full-region 10% scenario artifacts
## Produces the prepared MATSim XMLs for a 10% full-region scenario.
## Auto-generated by scripts/setup_drt_scenario.py --with-phase-a
##
## run_matsim: false here because eqasim's matsim.simulation.run hardcodes
## lastIteration=1, which doesn't produce stable congested travel times.
## The actual travel-time extraction happens via
## scripts/run_travel_time_extraction.sh after this config completes —
## that launches RunSimulation with lastIteration=40.
## Uses IDF default mode choice parameters (Hörl & Balac 2021) — no calibration.
...
config:
  ...
  java_memory: 48G
  mode_choice: false
  run_matsim: false   # synpp only produces prepared XMLs; see comment above
...
def main() -> None:
    ...
    print("=" * 70)
    if args.with_phase_a:
        print("Phase A (two steps):")
        print("  1a. Prepared XMLs (~1.5 h synthesis, cache-warm re-run ~2 min):")
        print("      .venv/Scripts/python -m synpp config_lyon_fullregion_10pct.yml")
        print("  1b. Multi-iteration travel-time extraction (~8 h for 40 iters):")
        print("      bash scripts/run_travel_time_extraction.sh [iterations=40]")
        print("      Output: output_fullregion_10pct/simulation_output/output_events.xml.gz")
        print()
